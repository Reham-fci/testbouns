...
class CartController extends Controller
{
    public function variant_price(Request $request)
    {
        ...
        if ($str != null) {
            ...
            for ($i = 0; $i < $count; $i++) {
                ...
                if ($variations[$i]->type == $str) {
                    ...
                    $tax = Helpers::tax_calculation($variations[$i]->price, $variations[$i]->tax, $variations[$i]->tax_type);
                    $discount = $variations[$i]->discount;
                    if($variations[$i]->discount_type == 'percent'){
                        $discount = round(($variations[$i]->discount * $variations[$i]->price) / 100 , 2);
                        
                    }
                    $price =  $variations[$i]->price - $discount + $tax;
                    $unitprice =  $variations[$i]->price;
                    $description =  $variations[$i]->description;
                    $quantity = intval($product->current_stock / $variations[$i]->numberOfPieces);
                }
            }
        } else {
        ...
        return [
            ...
            'price_after_tax_and_discount' => \App\CPU\Helpers::currency_converter($price),
            'discount' => \App\CPU\Helpers::currency_converter($discount),
            'discountValue' => $discount,
            'tax' => \App\CPU\Helpers::currency_converter($tax),
            'quantity' => $quantity,
            'unitprice' => \App\CPU\Helpers::currency_converter($unitprice),
    ...
    function removeAllCarts(){
        $beforeDate = date('Y-m-d H:i:s',strtotime(date('Y-m-d H:i:s'). "- 24 hours"));
        $query ="DELETE from carts 
        where cart_group_id in 
        (
        	select cart_group_id from 
        	(
                SELECT cart_group_id , max(created_at)  created_at FROM `carts` 
