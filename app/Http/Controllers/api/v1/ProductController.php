...
class ProductController extends Controller
{
    ...
    public function TellMe(Request $request){
        try{
            $user = Helpers::get_customer($request);
            // SELECT `id`, `productId`, `variation`, `qty` FROM `CustomerProductTell` WHERE 1
            $variation = DB::table('product_variations')->where('type' , $request->variation)->where('productId' , $request->productId)->first();
            $qty = isset($variation->numberOfPieces) ? $variation->numberOfPieces : 1;
            DB::table('CustomerProductTell')->insert([
    ...
    public function sendNotificationWhenHaveQuantity(){
        $title = "{المنتج}-{النوع}\n اللى كنت محتاجة موجود دلوقتى على بونص";
        $usersTells = DB::table('CustomerProductTell')
        ->select(DB::raw('products.id ,CustomerProductTell.id as CustomerProductTell_id , users.cm_firebase_token , products.name , products.thumbnail , CustomerProductTell.variation'))
        ->join('products' , 'products.id' , '=' , 'CustomerProductTell.productId')
        ->join('users' , 'users.id' , '=' , 'userId')
        ->where(DB::raw('floor(products.current_stock / CustomerProductTell.qty)') , '>' , 0)->get();
        
        $id = [0];
        foreach($usersTells as $usersTell){
            $notification = new Notification;
            $notification->description = str_replace('{المنتج}', $usersTell->name , $title);
            $notification->description = str_replace('{النوع}', $usersTell->variation ,$notification->description);
            $notification->title = "";
            $notification->image = $usersTell->thumbnail;;
    
            $notification->productId = $usersTell->id;
            $notification->status = 1;
            
            $id[] = $usersTell->CustomerProductTell_id;
            
            Helpers::send_push_notif_to_device($usersTell->cm_firebase_token, $notification);
            
        }
        
        DB::table('CustomerProductTell')->whereIn('id' , $id)->delete();
