...
<head>
    ...
    s.parentNode.insertBefore(t,s)}(window, document,'script',
    'https://connect.facebook.net/en_US/fbevents.js');
    fbq('init', '1426864458049539');
    fbq('track', 'PageView');
    </script>
    <noscript><img height="1" width="1" style="display:none"
    src="https://www.facebook.com/tr?id=1426864458049539&ev=PageView&noscript=1"
    /></noscript>
    <!-- End Meta Pixel Code -->
</head>
<!-- Body-->
...
<script>
    ...
    function updateQuantity2(key, element) {
        $.post('<?php echo e(route('cart.updateQuantity')); ?>', {
            _token: '<?php echo e(csrf_token()); ?>',
            key: key,
        ...
        if ($('#add-to-cart-form input[name=quantity]').val() > 0 && checkAddToCartValidity()) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $('.product-variation-description').html(data.description);
...
<script>
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.status == 1) {
                    ...
                    $("#coupon-discount-amount").text("-"+data.discount);
                    ms.forEach(
                        function (m, index) {
    ...
    function apply_wallet() {
        ...
        var text = "{{\App\CPU\Helpers::currency_converter(0)}}";
        var text = text.replace("0.00",amountfloat);
        $('.wallet_value').text(text)
        $('#form_submit_wallet').val(amount)
    }
    @endif
