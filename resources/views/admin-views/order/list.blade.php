                        ...
                        <form action="{{ url()->current() }}" method="GET">
                            ...
                            <div class="row">
                                ...
                                <div class="col-12 col-sm-5 mt-2 mt-sm-0">
                                    ...
                                    <select class="form-select form-control" name="salesPersonId" type="salesPersonId" id="si-salesPersonId"
                                        ...
                                        @foreach ($Salesperson as $_Salesperson)
                                            <option {{($salesPersonId == $_Salesperson->id) ? "selected" : ""}} value="{{$_Salesperson->id}}">{{$_Salesperson->f_name.' '.$_Salesperson->l_name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="col-12 col-sm-1 mt-1 mt-sm-0  ">
                                    <button class="btn-export btn btn-primary float-right float-sm-none">{{\App\CPU\translate('export')}}</button>
                                </div>
                            </div>
                        </form>
                    </div>
                    ...
                    @foreach($orders as $key=>$order)
                        ...
                        <tr class="status-{{$order['order_status']}} class-all">
                            ...
                            <td>
                                ...
                                <a class="btn btn-secondary btn-sm mr-1" title="{{\App\CPU\translate('view')}}"
                                    href="{{route('admin.orders.order-update',['id'=>$order['id']])}}"><i
                                        class="tio-edit"></i> </a>
...
@push('script_2')
    <script>
        ...
        $('.btn-export').on('click',function(e){
            ...
            var url = `search=`+search+`&from=`+from+`&to=`+to+`&salesPersonId=`+salesPersonId;
            console.log("<?=URL::to('admin/orders/export');?>"+url );
            open("<?=URL::to('admin/orders/export/'.$status.'?');?>"+url , '_blank');
        })
