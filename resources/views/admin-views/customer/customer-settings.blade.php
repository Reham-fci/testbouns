        ...
        <div class="card-body">
            <form action="{{ route('admin.customer.customer-settings-update') }}" method="post" enctype="multipart/form-data" id="update-settings">
                ...
                <div class="row pt-2 loyalty-point-section">
                    ...
                    <div class="col-sm-6 col-12">
                        <div class="form-group">
                            <label class="input-label" for="intem_purchase_point">{{\App\CPU\translate('walletOrderWithRate')}}</label>
                            <select  class="form-control" name="walletOrderWithRate">
                                <option <?php if($data['walletOrderWithRate'] == "0"){echo "selected";}?> value="0">{{\App\CPU\translate('No')}}</option>
                                <option <?php if($data['walletOrderWithRate'] == "1"){echo "selected";}?> value="1">{{\App\CPU\translate('yes')}}</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-sm-6 col-12">
                        <div class="form-group">
                            <label class="input-label" for="intem_purchase_point">{{\App\CPU\translate('walletOrderValue')}}</label>
                            <input type="number" class="form-control" name="walletOrderValue" min="0" step="0.1" value="{{$data['walletOrderValue']??'0'}}">
                        </div>
                    </div>
                </div>
