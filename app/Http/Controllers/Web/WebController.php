...
class WebController extends Controller
{
    ...
    {
        $socialMedia = [];
        $socialMediaObject = DB::table('business_settings')->whereIn('type',
            [
                'bonus_facebook',
                'bonus_WhatsApp',
                'bonus_phone',
        ...
        foreach($socialMediaObject as $_socialMedia){
            $socialMedia[$_socialMedia->type] = $_socialMedia->value;
        }
        ...
        return view('web-views.home', compact('socialMedia','featured_products', 'topRated', 'bestSellProduct', 'latest_products', 'categories', 'brands', 'deal_of_the_day', 'top_sellers', 'home_categories'));
    }
    ...
    public function all_categories($id=0)
    {
        ...
        return view('web-views.categories', compact('categories' , 'view' ,'id'));
    }
    ...
    public function checkout_Limit(Request $request){
        $area = $request->area; 
        $city = $request->city; 
        $sub_total = 0;
        $total_tax = 0;
        ...
        $Limit = isset($result[0]) ?  $result[0]->Limit : 0;
        $res['status'] = ($Limit <= $total ) ? 1 : 0;
        $res['limit'] = $Limit;
    ...
    public function checkout_details(Request $request)
    {
        ...
        if (count($cart_group_ids) > 0) {
            return view('web-views.checkout-shipping', compact('ispayment'  ,'governorates' , 'cities'));
    ...
    public function checkout_payment()
    {
        ...
        $wallet = auth('customer')->user()->wallet_balance;
        $walletValuesAndRates = [];
        $_walletValuesAndRates = DB::table('business_settings')->whereIn('type',
            [
                'walletOrderValue',
                'walletOrderWithRate',
            ]
        )->get();
        foreach($_walletValuesAndRates as $_walletValueAndRate){
            $walletValuesAndRates[$_walletValueAndRate->type] = $_walletValueAndRate->value;
        }
        
        
        if($walletValuesAndRates['walletOrderWithRate'])
        $max_amount = round(($walletValuesAndRates['walletOrderValue'] * $wallet)/100 , 2);
        else
        $max_amount = $walletValuesAndRates['walletOrderValue'];
     
        $max_amount = $wallet > $max_amount ? $max_amount : $wallet;
        ...
        if (session()->has('address_id') && count($cart_group_ids) > 0) {
            return view('web-views.checkout-payment', compact('ispayment','wallet','walletValuesAndRates','max_amount'));
        }
    ...
    { 
        ...
        DB::enableQueryLog();
        $usr = auth('customer')->user();
        $Gift = [];
        $cartItems = CartManager::get_cart_group_ids() ;
        $walletAmount = 0;
        if($cartItems){
        ...
        if($order_id == -1){
            ...
            return redirect()->route('shop-cart')
                ->with('error', 'Stock doesn\'t have enough Quentity');
        }
    ...
    public function shop_cart(Request $request)
    {
        ...
        if (auth('customer')->check() && Cart::where(['customer_id' => auth('customer')->id()])->count() > 0) {
            return view('web-views.shop-cart' , compact('ispayment'));
        }
        Toastr::info(translate('no_items_in_basket'));
        return redirect('/');
    }
