                            ...
                            <li id="choose_delivery_man">
                                ...
                                <select class="form-control text-capitalize js-select2-custom" name="delivery_man_id" onchange="addDeliveryMan(this.value)">
                                    ...
                                    @foreach($delivery_men as $deliveryMan)
                                        <option
                                            value="{{$deliveryMan['id']}}" {{$customer['salesPersonId']==$deliveryMan['id']?'selected':''}}>
...
function addDeliveryMan(id) {
            $.ajax({
                type: "GET",
                url: '{{url('/')}}/admin/customer/add-Salesperson/{{$customer['id']}}/' + id,
                data: {
                ...
                success: function (data) {
                    if (data.status == true) {
                        toastr.success('Salesperson successfully assigned/changed', {
                            CloseButton: true,
                            ProgressBar: true
                        });
                    } else {
                        toastr.error('Salesperson can not assign/change in that status', {
                            CloseButton: true,
                            ProgressBar: true
                        });
                    }
