...
class Helpers
{
    ...
    public static function set_data_format($data)
    {
        ...
        try {
            ...
            // $choice_options = json_decode($data['choice_options']);
            // $_choice_options = [];
            // unset($data['choice_options']);
            $variationName = [];
            foreach (json_decode($data['variation'], true) as $var) {
                // echo ($data['current_stock'] / $var['numberOfPieces']);exit;
                if(in_array($var['type'] , $variationName)) continue;
                $variationName [] = $var['type'];
                // $_choice_options[] = $var['type'];
                $variation[] = 
                [
                    'type' => $var['type'],
            ...
            // $d =  [] ;
            // $d[0] = [
            //         'name' => $choice_options[0]->name,
            //         'title' => $choice_options[0]->title,
            //         'options' => $_choice_options,
            // ];
            // $data['choice_options'] = $d;
            // $data['choice_options'][0]['title'] = $choice_options[0]->title;
            // $data['choice_options'][0]['options'] = $_choice_options;
            // $data['choice_options'][0]->options = '';
            // $data['choice_options'][0]->test = 1;
